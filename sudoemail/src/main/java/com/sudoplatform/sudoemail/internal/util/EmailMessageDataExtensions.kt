...
/**
 * An interface defining the email body [jakarta.mail.Part] and convenience functions for transforming
...
internal data class ViewableBodyPart(
    ...
    override fun asPlainText(context: Context): String? =
        when {
            part.isMimeType(MimeTypes.Companion.TEXT_ANY) && part.content is String -> part.content as String
            else -> {
                null
            }
        }
    ...
    override fun asHtmlText(context: Context): String? =
        when {
            part.isMimeType(MimeTypes.Companion.TEXT_HTML) -> {
                try {
            ...
            part.isMimeType(MimeTypes.Companion.TEXT_ANY) -> {
 ...
 * @property attachments [List<EmailAttachment>] A list of collected [com.sudoplatform.sudoemail.types.EmailAttachment]s.
 */
