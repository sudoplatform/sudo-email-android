...
/**
 * Test the correct operation of [SudoEmailClient.sendMaskedEmailMessage]
 * using mocks and spies.
 */
@RunWith(RobolectricTestRunner::class)
class SudoEmailSendMaskedEmailMessageTest : BaseTests() {
    private val emailMaskId = "mock-mask-id"
    ...
    private val headers =
        InternetMessageFormatHeader(
            EmailMessage.EmailAddress("mask@bar.com"),
    ...
    @Test
    fun `sendMaskedEmailMessage() should return results when no error present`() =
        runTest {
            val deferredResult =
                async(StandardTestDispatcher(testScheduler)) {
                    client.sendMaskedEmailMessage(input)
                }
            ...
            verify(mockUseCase).execute(
                check { useCaseInput ->
                    useCaseInput.senderEmailAddressId shouldBe null
                    useCaseInput.senderMaskId shouldBe emailMaskId
    ...
    @Test
    fun `sendMaskedEmailMessage() should throw error when use case throws`() =
        runTest {
            mockUseCase.stub {
                onBlocking {
                    execute(
