...
/**
 * Test the correct operation of [ListEmailAddressesForSudoIdUseCase]
 * using mocks and spies.
 */
@RunWith(RobolectricTestRunner::class)
class ListEmailAddressesForSudoIdUseCaseTest : BaseTests() {
    ...
    private val mockEmailAddressService by before {
        mock<EmailAddressService>().stub {
            onBlocking { listForSudoId(any()) } doReturn
    ...
    fun `execute() should return success result with unsealed email addresses`() =
        runTest {
            ...
            verify(mockEmailAddressService).listForSudoId(
                com.sudoplatform.sudoemail.internal.domain.entities.emailAddress.ListEmailAddressesForSudoIdRequest(
                    sudoId = mockSudoId,
                    limit = null,
                    nextToken = null,
                ),
    ...
    fun `execute() should return empty success result when no email addresses exist`() =
        runTest {
            ...
            verify(mockEmailAddressService).listForSudoId(any())
        }
        ...
        runTest {
            ...
            when (result) {
                is ListAPIResultEntity.Partial -> {
                    ...
                    partialResult.items[1].id shouldBe "id2"
                    partialResult.failed.size shouldBe 1
                    partialResult.failed[0].partial.id shouldBe "id3"
    ...
    fun `execute() should throw when service throws`() =
        runTest {
            mockEmailAddressService.stub {
                onBlocking { listForSudoId(any()) } doThrow RuntimeException("Service failed")
            }
    ...
    @Test
    fun `execute() should use correct mockSudoId in service request`() =
        runTest {
            val customSudoId = "custom-sudo-id-123"
            val input =
                ListEmailAddressesForSudoIdUseCaseInput(
                    sudoId = customSudoId,
                    limit = null,
