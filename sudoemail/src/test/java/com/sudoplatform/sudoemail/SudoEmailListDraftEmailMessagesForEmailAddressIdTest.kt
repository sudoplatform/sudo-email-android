...
class SudoEmailListDraftEmailMessagesForEmailAddressIdTest : BaseTests() {
    ...
    // E2EE path
    @Test
    fun `listDraftEmailMessagesForEmailAddressId() should throw FailedException if message has keyAttachments but no bodyAttachment`() =
        runTest {
            val emailAddressId = "emailAddressId"
    ...
    @Test
    fun `listDraftEmailMessagesForEmailAddressId() should return proper data for E2EE message if no errors`() =
        runTest {
            val emailAddressId = "emailAddressId"
            mockEmailMessageProcessor.stub {
                on { parseInternetMessageData(any()) } doReturn inNetworkEmailMessage
